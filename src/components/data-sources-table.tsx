...
const CATEGORIES = ["Todas", "Hidrologia", "Meteorologia", "Mercado Electrico", "Geoespacial", "Teledeteccion", "Calidad Agua", "Biodiversidad", "Geologia", "Solar/Eolico", "Socioeconomico", "Infraestructura", "Regulatorio"];
...
const SOURCES: Source[] = [
  ...
  { name: "Copernicus DEM", category: "Geoespacial", variables: "Elevacion 30m (mas moderno)", url: "https://registry.opendata.aws", cost: "Gratuito", status: "ingerido" },
  ...
  { name: "ALOS PALSAR", category: "Geoespacial", variables: "Elevacion DSM (banda L)", url: "https://search.asf.alaska.edu", cost: "Gratuito", status: "ingerido" },
  ...
  { name: "Maxar", category: "Teledeteccion", variables: "Imagenes 31cm", url: "https://www.maxar.com", cost: "~$15-25/km2", status: "pago" },
  { name: "CORANTIOQUIA", category: "Calidad Agua", variables: "pH, OD, DBO, turbidez", url: "https://www.corantioquia.gov.co", cost: "Gratuito", status: "pendiente" },
  ...
  { name: "GBIF / SiB", category: "Biodiversidad", variables: "Registros de especies", url: "https://api.gbif.org", cost: "Gratuito", status: "fase2" },
  ...
  { name: "Global Wind Atlas", category: "Solar/Eolico", variables: "Velocidad viento 10-200m", url: "https://globalwindatlas.info", cost: "Gratuito", status: "fase2" },
  ...
  { name: "INVIAS Red Vial", category: "Infraestructura", variables: "Vias, estado, trafico", url: "https://hermes2.invias.gov.co", cost: "Gratuito", status: "fase2" },
...
const STATUS_BADGE = {
  ingerido: { label: "Ingerido", cls: "bg-accent-light text-accent" },
  pendiente: { label: "Pendiente", cls: "bg-[#FFF8E1] text-[#B8860B]" },
  fase2: { label: "Fase 2+", cls: "bg-panel text-muted" },
  pago: { label: "Pago", cls: "bg-[#FDECEA] text-[#C0392B]" },
};

export default function DataSourcesTable() {
  const [filter, setFilter] = useState("Todas");
  const filtered = filter === "Todas" ? SOURCES : SOURCES.filter((s) => s.category === filter);
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 md:px-10 py-20 bg-panel">
      <AnimatedSection>
        ...
        <p className="font-[family-name:var(--font-sans)] text-muted mt-2 mb-8">
          {SOURCES.length} fuentes en 13 categorias — ~95% gratuitas
        </p>
      </AnimatedSection>
        ...
        <div className="flex flex-wrap gap-1.5 mb-6">
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-500 transition-all duration-500 ${
                filter === cat
                  ? "bg-text text-white"
                  : "bg-card text-muted border border-border hover:border-border-strong"
              }`}
            >
              {cat}
            </button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left">
                <th className="px-4 py-3 font-[family-name:var(--font-mono)] text-[10px] text-muted uppercase tracking-[0.15em]">Fuente</th>
                <th className="px-4 py-3 font-[family-name:var(--font-mono)] text-[10px] text-muted uppercase tracking-[0.15em]">Categoria</th>
                <th className="px-4 py-3 font-[family-name:var(--font-mono)] text-[10px] text-muted uppercase tracking-[0.15em] hidden md:table-cell">Variables</th>
                <th className="px-4 py-3 font-[family-name:var(--font-mono)] text-[10px] text-muted uppercase tracking-[0.15em]">Costo</th>
                <th className="px-4 py-3 font-[family-name:var(--font-mono)] text-[10px] text-muted uppercase tracking-[0.15em]">Estado</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {filtered.map((s) => {
                ...
                return (
                  <tr key={s.name} className="hover:bg-card-hover transition-colors duration-300">
                    <td className="px-4 py-3 font-500 text-text">
                      <a href={s.url} target="_blank" rel="noopener noreferrer" className="hover:text-accent transition-colors duration-300">
                        {s.name}
                      </a>
                    </td>
                    <td className="px-4 py-3 text-text-body">{s.category}</td>
                    <td className="px-4 py-3 text-muted hidden md:table-cell text-xs">{s.variables}</td>
                    <td className="px-4 py-3 text-text-body">{s.cost}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-block px-2 py-0.5 rounded text-[11px] font-500 ${badge.cls}`}>
                        {badge.label}
                      </span>
                    </td>
                  </tr>
